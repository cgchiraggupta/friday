...
export async function get_blend_pools(): Promise<BlendPool[]> {
  try {
    ...
    const response = await axios
      .get<BlendApiResponse>("https://api.blend.capital/pools", {
        timeout: 5000,
      })
      .catch(() => null);
...
export async function build_deposit_tx(
  ...
  try {
    ...
    throw new Error(`Could not build deposit transaction: ${message}`);
  }
}
...
export async function build_withdraw_tx(
  ...
  try {
    ...
    throw new Error(`Could not build withdrawal transaction: ${message}`);
  }
}
